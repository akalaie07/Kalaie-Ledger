...
export type ImportFormOptions = {
  ...
  salesPartners: { id: string; name: string }[];
};
...
export async function getImportFormOptions(): Promise<ImportFormOptions> {
  ...
  const [{ data: platforms }, { data: products }, { data: closers }, { data: salesPartners }] =
    await Promise.all([
      ...
      supabase.from("sales_partners").select("id, name").eq("organization_id", session.organizationId).eq("active", true).order("name"),
    ]);
