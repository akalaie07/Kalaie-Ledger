...
export const metadata: Metadata = { title: "Inkasso — Kalaie Ledger" };
...
export default async function InkassoPage() {
              ...
              <tr key={r.id} className="hover:bg-muted/30 transition-colors">
                ...
                <td className="px-4 py-3">
                  <span className={cn(
                    ...
                    STATUS_CLASS[r.computed_status ?? "in_collection"] ?? "bg-muted text-muted-foreground",
                  )}>
                    {STATUS_LABEL[r.computed_status ?? "in_collection"] ?? r.computed_status}
                  </span>
                </td>
            ...
            {rows.length === 0 && (
              <tr>
                <td colSpan={6} className="px-4 py-12 text-center text-muted-foreground">
                  <p className="text-sm">Keine Deals in Inkasso.</p>
